...
// Genuine cmdk primitives — imported directly (no @/components/ui/command wrapper,
// no custom search/menu substitute). The official registry ModelSelector only
// needs these six subcomponents.
...
const CommandSeparator = Command.Separator;
...
export const modelSelectorTriggerVariants = cva(
  "focus-visible:ring-ring/50 flex cursor-pointer w-fit items-center justify-between gap-2 overflow-hidden rounded-md text-sm whitespace-nowrap transition-colors outline-none focus-visible:ring-1 disabled:cursor-not-allowed disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5",
  {
    variants: {
      variant: {
        outline:
...
// Standalone, controlled registry ModelSelector. The connected variant that
// registered selection with assistant-ui's ModelContext is intentionally NOT
// included; this component is driven entirely by props (value / onValueChange /
// effort / onEffortChange) so the host owns authoritative selection state.
