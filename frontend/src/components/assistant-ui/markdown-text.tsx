  ...
  return (
    <div className="aui-code-header-root border-border/50 bg-muted/50 mt-3 flex items-center justify-between rounded-t-xl border border-b-0 px-3.5 py-1.5 text-xs">
      ...
      <button
        ...
        title="Copy"
        onClick={onCopy}
        className="grid h-6 w-6 place-items-center rounded-md text-muted-foreground transition-colors hover:bg-interactive-hover hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-interactive-focus-ring"
      >
        {!isCopied && (
          <CopyIcon className="h-3.5 w-3.5 animate-in zoom-in-75 fade-in duration-150" />
        )}
        {isCopied && (
          <CheckIcon className="h-3.5 w-3.5 animate-in zoom-in-50 fade-in duration-200 ease-out" />
        )}
      </button>
    </div>
  );
