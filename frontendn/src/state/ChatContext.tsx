...
// Parts keep the chronological order of a turn: thinking, tool activity and
// reply text each append to this list as they arrive, so the transcript reads
// in the order things actually happened.
function upsertPart(message: Message, part: MessagePart): Message {
...
function appendPartText(message: Message, partId: string, text: string): Message {
  const parts = message.parts ? [...message.parts] : [];
  const idx = parts.findIndex((p) => p.id === partId);
  if (idx >= 0) {
    parts[idx] = { ...parts[idx], text: parts[idx].text + text };
  } else {
    // A delta with no preceding part_start: open the part rather than drop it.
    parts.push({ id: partId, type: "text", text, done: false });
  }
  return { ...message, parts };
}

// Tool updates must land in both places: the legacy tools array and the part
// timeline. The part is what puts the tool in chronological position.
function syncToolPart(message: Message, tool: ToolCall): Message {
  const withTool = appendTool(message, tool);
  const parts = withTool.parts ? [...withTool.parts] : [];
  const idx = parts.findIndex((p) => p.id === tool.id);
  if (idx >= 0) {
    parts[idx] = { ...parts[idx], tool: { ...parts[idx].tool, ...tool } as ToolCall };
  } else {
    parts.push({ id: tool.id, type: "tool", text: "", done: false, tool });
  }
  return { ...withTool, parts };
}
...
function reduceEvent(state: ChatState, event: ServerEvent): ChatState {
  switch (event.type) {
    ...
    case "part_delta": {
      ...
      const messages = state.messages.map((m) =>
        m.id === state.activeMessageId
          ? appendPartText(m, event.part_id, event.text || "")
          : m,
      );
      return { ...state, messages };
    }
    ...
    case "part_done": {
      ...
      const messages = state.messages.map((m) => {
        ...
        const parts = (m.parts ?? []).map((p) =>
          p.id === event.part_id ? { ...p, done: true } : p,
        );
        return { ...m, parts };
      });
      return { ...state, messages };
    ...
    case "done": {
      ...
      const messages = state.messages.map((m) => {
        ...
        // The turn is over, so nothing is still streaming.
        const parts = m.parts?.map((p) => (p.done ? p : { ...p, done: true }));
        return { ...m, status: "complete" as const, tools, parts };
      });
