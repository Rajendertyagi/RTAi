...
def _fake_acp_module() -> tuple[ModuleType, ModuleType]:
    ...
    def embedded_text_resource(uri: str, text: str, *, mime_type: str | None = None) -> dict:
        return {"_kind": "embedded_text_resource", "uri": uri, "text": text, "mime_type": mime_type}

    def embedded_blob_resource(uri: str, blob: str, *, mime_type: str | None = None) -> dict:
        return {"_kind": "embedded_blob_resource", "uri": uri, "blob": blob, "mime_type": mime_type}
...
class PromptContentDomainTests(unittest.TestCase):
    ...
    def test_text_block_normalized(self) -> None:
        ...
        self.assertEqual(content.size_bytes, len("hello".encode("utf-8")))

    def test_image_block_normalized(self) -> None:
        raw = b"\x89PNG"
        b64 = base64.b64encode(raw).decode("ascii")
        content = make_prompt_content({"kind": "image", "name": "img.png", "mime_type": "image/png", "data_base64": b64})
    ...
    def test_audio_block_normalized(self) -> None:
        raw = b"\x00RIFF"
        b64 = base64.b64encode(raw).decode("ascii")
        content = make_prompt_content({"kind": "audio", "name": "aud.wav", "mime_type": "audio/wav", "data_base64": b64})
        self.assertEqual(content.kind, PromptKind.AUDIO)
        self.assertEqual(content.data, raw)

    def test_resource_link_normalized(self) -> None:
        content = make_prompt_content({"kind": "resource_link", "name": "r.txt", "uri": "https://example.com/r.txt", "mime_type": "text/plain"})
    ...
    def test_embedded_text_normalized(self) -> None:
        content = make_prompt_content({"kind": "embedded_text", "name": "e.txt", "mime_type": "text/plain", "text": "inline"})
    ...
    def test_embedded_blob_normalized(self) -> None:
        ...
        content = make_prompt_content({"kind": "embedded_blob", "name": "b.zip", "mime_type": "application/zip", "data_base64": b64})
    ...
    def test_name_is_stripped(self) -> None:
        content = make_prompt_content({"kind": "text", "name": "  spaced  ", "text": "x"})
    ...
    def test_text_size_uses_utf8_bytes(self) -> None:
        ...
        self.assertEqual(content.size_bytes, len("héllo".encode("utf-8")))
...
class PromptContentRejectionTests(unittest.TestCase):
    ...
    def test_mutual_exclusion_per_kind(self) -> None:
        cases: list[tuple[str, dict]] = [
            ("text+data_base64", {"kind": "text", "name": "t", "text": "x", "data_base64": "a"}),
            ("text+uri", {"kind": "text", "name": "t", "text": "x", "uri": "file:///x"}),
            ("image+text", {"kind": "image", "name": "t", "data_base64": "a", "mime_type": "image/png", "text": "x"}),
            ("image+uri", {"kind": "image", "name": "t", "data_base64": "a", "mime_type": "image/png", "uri": "file:///x"}),
            ("audio+text", {"kind": "audio", "name": "t", "data_base64": "a", "mime_type": "audio/wav", "text": "x"}),
            ("audio+uri", {"kind": "audio", "name": "t", "data_base64": "a", "mime_type": "audio/wav", "uri": "file:///x"}),
            ("resource_link+text", {"kind": "resource_link", "name": "t", "uri": "file:///x", "text": "x"}),
            ("resource_link+b64", {"kind": "resource_link", "name": "t", "uri": "file:///x", "data_base64": "a"}),
            ("embedded_text+b64", {"kind": "embedded_text", "name": "t", "text": "x", "mime_type": "text/plain", "data_base64": "a"}),
            ("embedded_text+uri", {"kind": "embedded_text", "name": "t", "text": "x", "mime_type": "text/plain", "uri": "file:///x"}),
            ("embedded_blob+text", {"kind": "embedded_blob", "name": "t", "data_base64": "a", "mime_type": "application/zip", "text": "x"}),
            ("embedded_blob+uri", {"kind": "embedded_blob", "name": "t", "data_base64": "a", "mime_type": "application/zip", "uri": "file:///x"}),
        ]
    ...
    def test_image_missing_data_base64_rejected(self) -> None:
        with self.assertRaises(PromptValidationError):
            make_prompt_content({"kind": "image", "name": "t", "mime_type": "image/png"})

    def test_image_missing_mime_rejected(self) -> None:
        with self.assertRaises(PromptValidationError) as ctx:
            make_prompt_content({"kind": "image", "name": "t", "data_base64": _IMAGE_B64})
        self.assertIn("mime_type", str(ctx.exception).lower())

    def test_audio_missing_mime_rejected(self) -> None:
        with self.assertRaises(PromptValidationError):
            make_prompt_content({"kind": "audio", "name": "t", "data_base64": _AUDIO_B64})
    ...
    def test_embedded_text_missing_text_rejected(self) -> None:
        with self.assertRaises(PromptValidationError):
            make_prompt_content({"kind": "embedded_text", "name": "t", "mime_type": "text/plain"})

    def test_embedded_blob_missing_data_base64_rejected(self) -> None:
        with self.assertRaises(PromptValidationError):
            make_prompt_content({"kind": "embedded_blob", "name": "t", "mime_type": "application/zip"})
    ...
    def test_image_non_string_b64_rejected(self) -> None:
        with self.assertRaises(PromptValidationError):
            make_prompt_content({"kind": "image", "name": "t", "mime_type": "image/png", "data_base64": b"raw"})

    # -- base64 validation --
    def test_empty_base64_rejected(self) -> None:
        with self.assertRaises(PromptValidationError):
            make_prompt_content({"kind": "image", "name": "t", "mime_type": "image/png", "data_base64": ""})

    def test_invalid_base64_rejected(self) -> None:
        with self.assertRaises(PromptValidationError):
            make_prompt_content({"kind": "image", "name": "t", "mime_type": "image/png", "data_base64": "!!!not-base64!!!"})

    def test_base64_non_canonical_rejected(self) -> None:
        with self.assertRaises(PromptValidationError):
            make_prompt_content({"kind": "image", "name": "t", "mime_type": "image/png", "data_base64": "aGVsbG8"})

    # -- MIME type validation --
    def test_image_unsupported_mime_rejected(self) -> None:
        with self.assertRaises(PromptValidationError):
            make_prompt_content({"kind": "image", "name": "t", "mime_type": "application/pdf", "data_base64": _IMAGE_B64})

    def test_audio_unsupported_mime_rejected(self) -> None:
        with self.assertRaises(PromptValidationError):
            make_prompt_content({"kind": "audio", "name": "t", "mime_type": "text/plain", "data_base64": _AUDIO_B64})

    def test_embedded_text_unsupported_mime_rejected(self) -> None:
        with self.assertRaises(PromptValidationError):
            make_prompt_content({"kind": "embedded_text", "name": "t", "mime_type": "image/png", "text": "x"})

    def test_embedded_blob_unsupported_mime_rejected(self) -> None:
        with self.assertRaises(PromptValidationError):
            make_prompt_content({"kind": "embedded_blob", "name": "t", "mime_type": "image/png", "data_base64": _IMAGE_B64})

    def test_embedded_text_accepted_mimes(self) -> None:
        for mime in ("text/plain", "text/csv", "text/html", "text/xml", "application/json", "application/pdf", "application/zip"):
            with self.subTest(mime=mime):
                content = make_prompt_content({"kind": "embedded_text", "name": "t", "mime_type": mime, "text": "x"})
    ...
    def test_embedded_blob_accepted_mimes(self) -> None:
        for mime in ("application/pdf", "application/zip"):
            with self.subTest(mime=mime):
                content = make_prompt_content({"kind": "embedded_blob", "name": "t", "mime_type": mime, "data_base64": _IMAGE_B64})
                self.assertEqual(content.mime_type, mime)

    # -- URI validation --
    def test_uri_credentials_rejected(self) -> None:
        with self.assertRaises(PromptValidationError):
            make_prompt_content({"kind": "resource_link", "name": "t", "uri": "file://user:pass@host/x"})

    def test_uri_http_scheme_rejected(self) -> None:
        with self.assertRaises(PromptValidationError):
            make_prompt_content({"kind": "resource_link", "name": "t", "uri": "http://example.com/x"})
    ...
    def test_uri_https_accepted(self) -> None:
        content = make_prompt_content({"kind": "resource_link", "name": "t", "uri": "https://example.com/x"})
        self.assertEqual(content.uri, "https://example.com/x")

    def test_uri_file_accepted(self) -> None:
        content = make_prompt_content({"kind": "resource_link", "name": "t", "uri": "file:///tmp/x"})
    ...
    def test_item_size_exceeds_limit(self) -> None:
        ...
        content = make_prompt_content({"kind": "image", "name": "big.png", "mime_type": "image/png", "data_base64": b64})
        with self.assertRaises(PromptValidationError):
    ...
    def test_total_size_exceeds_limit(self) -> None:
        ...
        contents = [
            make_prompt_content({"kind": "image", "name": "a.png", "mime_type": "image/png", "data_base64": b64}),
            make_prompt_content({"kind": "image", "name": "b.png", "mime_type": "image/png", "data_base64": b64}),
        ]
        with self.assertRaises(PromptValidationError):
            validate_prompt_limits(contents)

    def test_block_count_exceeds_limit(self) -> None:
        contents = [make_prompt_content({"kind": "text", "name": f"t{i}", "text": "x"}) for i in range(DEFAULT_MAX_COUNT + 1)]
        with self.assertRaises(PromptValidationError):
            validate_prompt_limits(contents)
    ...
    def test_error_no_raw_base64_leak(self) -> None:
        ...
        with self.assertRaises(PromptValidationError) as ctx:
            make_prompt_content({"kind": "image", "name": "t.png", "mime_type": "image/png", "data_base64": bad_b64})
        self.assertNotIn(bad_b64, str(ctx.exception))

    def test_supported_embedded_text_mimes(self) -> None:
        for mime in ("text/plain", "text/csv", "text/html", "text/xml", "application/json", "application/pdf", "application/zip"):
            with self.subTest(mime=mime):
                c = make_prompt_content({"kind": "embedded_text", "name": "t", "mime_type": mime, "text": "x"})
...
class CapabilityNegotiationTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_image_rejected_when_capability_false(self) -> None:
        ...
        caps.attachment_images = False
        session = _make_session(caps)
        content = [make_prompt_content({"kind": "image", "name": "t.png", "mime_type": "image/png", "data_base64": _IMAGE_B64})]
    ...
    async def test_audio_rejected_when_capability_false(self) -> None:
        ...
        caps.attachment_audio = False
        session = _make_session(caps)
        content = [make_prompt_content({"kind": "audio", "name": "t.wav", "mime_type": "audio/wav", "data_base64": _AUDIO_B64})]
        with self.assertRaises(RuntimeError) as ctx:
            await session.submit_prompt_content(content)
        ...
        for kind in (PromptKind.EMBEDDED_TEXT, PromptKind.EMBEDDED_BLOB):
            with self.subTest(kind=kind.value):
                ...
                if kind == PromptKind.EMBEDDED_TEXT:
                    data = {"kind": "embedded_text", "name": "t", "mime_type": "text/plain", "text": "x"}
                else:
                    data = {"kind": "embedded_blob", "name": "t", "mime_type": "application/zip", "data_base64": _IMAGE_B64}
    ...
    async def test_resource_link_always_supported(self) -> None:
        ...
        caps.attachment_embedded = False
        session = _make_session(caps)
        content = [make_prompt_content({"kind": "resource_link", "name": "t.txt", "uri": "file:///x"})]
        try:
            await session.submit_prompt_content(content)
        except RuntimeError as exc:
            self.assertNotIn("attachment rejected", str(exc))
    ...
    async def test_server_adapter_reports_no_attachments(self) -> None:
        ...
        self.assertEqual(snap.attachments.reason, UnavailabilityReason.NOT_EXPOSED_BY_PROVIDER)
...
class AcpBlockConversionTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_image_converts_to_image_block(self) -> None:
        ...
        session = _make_session(caps)
        content = [make_prompt_content({"kind": "image", "name": "img.png", "mime_type": "image/png", "data_base64": _IMAGE_B64})]
        try:
            await session.submit_prompt_content(content)
        except AttributeError:
            pass
    ...
    async def test_audio_converts_to_audio_block(self) -> None:
        ...
        session = _make_session(caps)
        content = [make_prompt_content({"kind": "audio", "name": "aud.wav", "mime_type": "audio/wav", "data_base64": _AUDIO_B64})]
        try:
            await session.submit_prompt_content(content)
        except AttributeError:
            pass
    ...
    async def test_resource_link_converts_to_resource_link_block(self) -> None:
        session = _make_session()
        content = [make_prompt_content({"kind": "resource_link", "name": "r.txt", "uri": "file:///tmp/r.txt", "mime_type": "text/plain"})]
        try:
            await session.submit_prompt_content(content)
        except AttributeError:
            pass
    ...
    async def test_embedded_blob_converts_correctly(self) -> None:
        ...
        caps.attachment_embedded = True
        session = _make_session(caps)
        content = [make_prompt_content({"kind": "embedded_blob", "name": "b.zip", "mime_type": "application/zip", "data_base64": _IMAGE_B64})]
        try:
            await session.submit_prompt_content(content)
        except AttributeError:
            pass
    ...
    async def test_block_order_preserved(self) -> None:
        ...
        content = [
            ...
            make_prompt_content({"kind": "image", "name": "img.png", "mime_type": "image/png", "data_base64": _IMAGE_B64}),
            make_prompt_content({"kind": "embedded_text", "name": "e.txt", "mime_type": "text/plain", "text": "inline"}),
        ]
        try:
            await session.submit_prompt_content(content)
        except AttributeError:
