...
from app.agents.capabilities import (
    AgentDescriptor, CapabilitySection, CapabilitySnapshot,
)
        ...
        try:
            with client.websocket_connect("/ws?cwd=/tmp") as ws:
                ...
                for _ in range(10):
                    ...
                    if ev.get("type") == "command_result":
                        continue
                    if ev.get("type") == "done" and ev.get("session_id") == "s1" and ev.get("turn_id") == "t1":
                        done_received = True
                        break

                self.assertTrue(done_received, "Expected done event with matching session/turn")
    ...
    async def test_cancel_unrelated_terminal_turn_rejected(self) -> None:
        ...
        try:
            with client.websocket_connect("/ws?cwd=/tmp") as ws:
                ...
                for _ in range(10):
                    ev = ws.receive_json()
                    if ev.get("type") == "command_result" and ev.get("request_id") == "req-prompt-1":
                        break
